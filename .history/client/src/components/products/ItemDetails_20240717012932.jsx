...
Memory updated
To optimize your ItemDetails component and address the issues you've mentioned, here's an updated version of the code. The primary focus is to ensure efficient data fetching, avoid unnecessary re-renders, and maintain UI consistency:

Prevent Unnecessary Fetching: Ensure data is fetched only when necessary and cached properly.
Minimize Re-Renders: Use useMemo and useCallback to avoid unnecessary re-renders.
Update UI Consistently: Ensure the UI reflects the current state accurately without flickering or inconsistent updates.
...
import { addToWatchlist, removeFromWatchlistAction } from '../../redux/action/watchlistAction';
...
function ItemDetails() {
  ...
  // Fetch product data
  const { isLoading, error, data: response } = useQuery(["product", id], () => fetchProductsById(id), {
  ...
  const handleIncreaseQuantity = useCallback(() => setQuantity((prevQuantity) => prevQuantity + 1), []);
  const handleDecreaseQuantity = useCallback(() => setQuantity((prevQuantity) => (prevQuantity > 1 ? prevQuantity - 1 : 1)), []);
  const handleShareClick = useCallback(() => setIsSharePopupVisible((prev) => !prev), []);
  ...
  }, [buttonText, dispatch, product, quantity, selectedColor, selectedSize, navigate]);
          ...
          <div className="item-desc">
                ...
                <div className="share-popup">
                  <div className="share-popup-content">
                    ...
                    <div className="share-popup-buttons">
                      <button onClick={() => setIsSharePopupVisible(false)}>Cancel</button>
            ...
            <p className="item-desc-rating">
              {Array(5).fill().map((_, index) => (
                <Star key={index} size={16} fill="rgb(4, 148, 4)" color="rgb(4, 148, 4)" />
              ))}
            ...
            <div className="item-color-variety">
              ...
              <ul className="color-variety-list">
                {colorOptions.map((color, index) => (
                  <li
                    key={index}
                    className={`color-palette ${selectedColor === color ? "selected" : ""}`}
            ...
            <div className="item-size-variety">
              ...
              <ul className="size-variety-list">
                {sizeOptions.map((size, index) => (
                  <li
                    key={index}
                    className={`size-palette ${selectedSize === size ? "selected" : ""}`}
...
export default ItemDetails;-cart"
                  onClick={handleAddToCart}
                >
                  {buttonText}
                </button>
