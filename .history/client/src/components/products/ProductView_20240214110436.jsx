...
import { Grid2X2, Star, MoveRight, Braces } from "lucide-react";
...
function ProductView() {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const isBrandMatch = selectedBrands.length === 0 ||
      selectedBrands.includes(product.brands);
    const isPriceMatch =
  ...
  const handleBrandChange = (event) => {
    const {value, checked } = event.target;
    if(checked &&!selectedBrands.includes(value)){
      setSelectedBrands([...selectedBrands, value]);
    }else {
      setSelectedBrands(selectedBrands.filter((brand) => br))
    }
  }
