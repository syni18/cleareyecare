...
function ItemDetails() {
  ...
  const { watchlistItems, addToWatchlist, removeFromWatchlist } = useWatchlistStore((state) => ({
  ...
  const handleIncreaseQuantity = useCallback(() => setQuantity((prev) => prev + 1), []);
  const handleDecreaseQuantity = useCallback(() => setQuantity((prev) => Math.max(prev - 1, 1)), []);
  ...
  const openSizeChart = useCallback(() => setIsSizeChartVisible(true), []);
  const closeSizeChart = useCallback(() => setIsSizeChartVisible(false), []);

  const handleToggleWatchlist = useCallback(() => {
    if (product?._id) {
      const isItemInWatchlist = watchlistItems.some((item) => item.productId === product._id);
      isItemInWatchlist ? removeFromWatchlist(product._id) : addToWatchlist(product._id);
    }
  }, [product?._id, watchlistItems, addToWatchlist, removeFromWatchlist]);
  ...
  }, [buttonText, addToCart, product, quantity, selectedColor, selectedSize, navigate]);
      ...
      <div className="item-description-container">
        ...
        <div className="item-images-desc">
          <div className="item-images">
            ...
            <img src={selectedImage} alt="Selected" className="images-target" />
          </div>
          <div className="item-desc">
            <div className="item-category-watchlist">
              <em>{product.category}</em>
              <span>
                <Forward className="item-share-btn" onClick={handleShareClick} />
                <Heart
                  fill={watchlistItems.some((item) => item.productId === product._id) ? "red" : "#666"}
                  stroke={watchlistItems.some((item) => item.productId === product._id) ? "red" : "#666"}
            ...
            <div className="item-size-variety">
              ...
              {isSizeChartVisible && (
                <>
                  ...
                  <SizeChart onClose={closeSizeChart} />
                </>
              )}
            </div>
