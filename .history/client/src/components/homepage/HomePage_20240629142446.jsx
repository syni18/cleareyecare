...
import fallbackImage from "../../assets/fallbackImage.png"; // Add a fallback image
...
function HomePage() {
  ...
  const { isLoading, error: queryError, data: response } = useQuery("products", fetchProducts);
  ...
  const handleToggleWatchlist = useCallback(
    (product) => {
      ...
      const isItemInWatchlist = watchlistItems.some((item) => item._id === product._id);
  ...
  const placeholders = useMemo(() => Array.from({ length: 10 }).map((_, index) => (
          ...
          : products.slice(start, end).map((product) => (
              <div className="product" key={product._id}>
                <span className="product-wishlist-add">
                  <Heart
                    fill={watchlistItems.some((item) => item._id === product._id) ? "red" : "#666"}
                    stroke={watchlistItems.some((item) => item._id === product._id) ? "red" : "#666"}
                ...
                <Link to={`/item/${product._id}`}>
                  <img
                    ...
                    onError={(e) => e.target.src = fallbackImage} // Handle image error
                  />
                </Link>
