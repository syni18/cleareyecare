...
import useWatchlistStore, { useWishlistStore } from "../../redux/store/watchlistStore";
              ...
              .map((product) => (
                <div className="product" key={product._id}>
                  <span className="product-wishlist-add">
                    <Heart
                      fill={
                        wishlists.some((item) => item.productId === product._id)
                          ? "red"
                          : "#666"
                      }
                      stroke={
...
function HomePage() {
  const {wishlists, setWishlists, addItemWishlists, removeItemWishlists } = useWishlistStore();
  ...
  const productGridProps = useMemo(
    ...
    [products, isLoading, placeholders, wishlists, handleToggleWatchlist]
  );
