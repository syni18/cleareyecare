...
console.log(`Watchlist item`, watchlistItems[0]);
  // Function to handle adding/removing from watchlist
  const handleToggleWatchlist = (product) => {
    const isItemInWatchlist = watchlistItems[0].watchlist.some(
        ...
        {products.slice(start, end).map((product) => (
          <div className="product" key={product._id}>
            <span className="product-wishlist-add">
              <Heart
                fill={
                  watchlistItems[0].watchlist.some((item) => item._id === product._id)
                    ? "red"
                    : "#666"
                }
                stroke={
                  watchlistItems[0].some((item) => item._id === product._id)
                    ? "red"
                    : "#666"
                }
