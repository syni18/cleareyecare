  ...
  useEffect(() => {
    const pandetails = async () => {
      try {
        ...
        if(getPan.status) {
          setFieldValue('fullname', getPan.dafullname);
          setFieldValue('panNumber', getPan.panNumber);
          setFieldValue('declaration', getPan.declaration);
          setImagePreview('panImage',getPan.panImage);
        }
      } catch (error) {
