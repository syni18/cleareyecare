...
function PANUpload() {
    ...
    onSubmit: async (values) => {
      try {
        const response = await toast.promise(addPanCard(values), {
          loading: "Logging...",
          success: (response) => {
            return <b>{response.msg || "Pancard verify Successfully!"}</b>;
          },
          error: (error) => {
            return <b>{error.msg || "Could not verify!"}</b>;
          },
        });
  ...
  const handleFileChange = (e) => {
    ...
    const compressedImage = await imageCompression(values.panImageFile, options);
