...
function ResetPassword() {
  ...
  const { state } = useLocation(); // Assuming state contains the user's email
   const { setIsOtpVerified } = useAuth();

   useEffect(() => {
     const handlePopState = (event) => {
       // Check if we are navigating away from reset-password
       if (location.pathname === "/reset-password") {
         setIsOtpVerified(false); // Clear OTP verification state
         navigate("/login"); // Redirect to recovery or any other page
       }
     };
