...
function NewAddress({ onCancel, mode, addressSavedData }) {
  ...
  const validateForm = () => {
    if (
      !statesOptions.fullName ||
      !statesOptions.phoneNumber ||
      !statesOptions.address ||
      !statesOptions.locality ||
      !statesOptions.landmark ||
      !statesOptions.cityDistrictTown ||
      !statesOptions.state
    ) {
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isFormValid) {
      try {
        ...
        const response = await addAddress(statesOptions);
