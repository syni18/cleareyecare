...
function NewAddress({ onCancel, mode, onAddAddress, addressSavedData, onUpdateAddress }) {
  const statesOptions = [
  ...
  const [address, setAddress] = useState(addressSavedData || {});

  const handleSave = () => {
    if (mode === "create") {
      // Assume `address` is the new address object
  ...
  const { handleSubmit, getFieldProps, setFieldValue, values } = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (onUpdateAddress) {
          onUpdateAddress(response.data); // Notify parent of updated address
                onAddAddress(address);
        }
        onCancel();
      } catch (error) {
