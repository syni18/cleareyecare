...
useEffect(() => {
  ...
  setAddressSavedData(addressToEdit); // Set the address details in the state
  setIsEditing(true);
  };
  const handleDelete = async (addressId) => {
    try {
      const res = await deleteSavedAddress(addressId);
      console.log(res);
      setOpenDropdownId(null); // Close the dropdown if the deleted address was open
    } catch (error) {
      console.error("Error deleting address:", error);
      // Handle error as needed, such as showing a notification to the user
    }
  };
