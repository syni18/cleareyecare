...
export async function getUser({fullname}){
    try{
        const {data} = await axios.get(`/vi/api/user/${fullname}`)
        return {data};
    }
    catch(error){
...
export async function updateUser(response){
    try{ 
        ...
        const data = await axios.put(`/api/updateuser`, response, {heaader: {"Authorization": `Bearer ${token}`}});
...
export async function generateOTP(fullname){
    try{
        const {data: {code}, status} =  await axios.get(`/api/generate-Otp`, {params: {fullname}});

        // send mail with otp
        if(status === 201){
            let {data: {email}} = await getUser({fullname})

            let text = `Your Password Recovery Otp is ${code}. Verify and Recover your Password`;
            await axios.post(`/api/registerMail`, {fullname, useremail: email, text, subject: "Password Recovery OTP"})
        }
...
export async function verifyOtp({fullname, code}){
    try{
        const {data , status} = await axios.get(`/api/verify-Otp`, {params: {fullname, code}})
        return {data, status}
    }catch(error){
        return Promise.reject(error);
    }
...
export async function resetPassword({fullname, password}){
    try{
        const {data, status} = await axios.put('/api/resetPassword', {fullname, password});
