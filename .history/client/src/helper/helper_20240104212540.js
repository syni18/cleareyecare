...
export async function generateOTP(email){
    try{
        const {data: {code}, status} =  await axios.get(`/v1/api/generate-Otp`);
        // console.log("code : ", code, status);
        // send mail with otp
        if(status === 201){

            let {data} = await authenticate(email);
            // console.log(data);

            let text = `Your Password Recovery OTP is ${code}. Verify and Recover your Password`;
            let subject = "Password Recovery OTP";
            await axios.post(`/v1/api/registerMail`, {username: data.fullname, useremail: data.email, text, subject})
