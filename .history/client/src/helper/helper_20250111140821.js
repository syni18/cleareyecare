...
export async function addProductToCart(product) {
  try {
    const response = await axios.post("v1/api/addToCart", product, {withCredentials: true});
...
export async function removeProductFromCart(product) {
  try {
    const response = await axios.delete("v1/api/removeFromCart", product, {
      withCredentials: true,
    });
    console.log("res remove to cart", response);
    return response.data;
  } catch (error) {
    console.error("Error adding product to cart: ", error);
  }
