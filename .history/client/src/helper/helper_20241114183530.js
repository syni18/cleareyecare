...
export async function registerUser({ credential }) {
  try {
    ...
    // Destructure fullname and email early
    ...
    const {
      data: { msg },
      status,
    } = await axios.post(`/v1/api/register`, credential);

    // Send email if registration is successful
    if (status === 201) {
      await axios.post(`/v1/api/registerMail`, {
        fullname,
        useremail: email,
        text: msg,
      });
    ...
    return msg; // No need for Promise.resolve since this is an async function
  } catch (error) {
    console.error(
      "Error in registerUser :",
      error.response ? error.response.data : error.message
    );
    throw new Error(error.response ? error.response.data : error.message); // Throwing an error for better handling
  }
}
