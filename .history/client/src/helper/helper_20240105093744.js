...
// ** generate OTP **
export async function generateOTP({ExistUser}){
    console.log(ExistUser);
    try{
        const {data: {code}, status} =  await axios.get(`/v1/api/generate-Otp`, {params: ExistUser.data.fullname});
        console.log("code : ", code, status);
        // send mail with otp
        if(status === 201){

            let {data: {email}} = await getUserDetail(ExistUser.data.fullname);

            let text = `Your Password Recovery OTP is ${code}. Verify and Recover your Password`;
            let subject = "Password Recovery OTP";
            await axios.post(`/v1/api/registerMail`, {username: ExistUser.data.fullname, useremail: email, text, subject})
