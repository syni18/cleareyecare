...
export async function deleteAddressById(req, res) {
  try {
    ...
    const { id: addressId } = req.params; // Address ID to delete
    ...
    if (userAddresses.defaultAddress?.id === addressId) {
    ...
    // If the default address was deleted, update it to the first address in the array
    if (isDefaultAddress) {
      const updatedUserAddresses = await AddressModel.findOne({
        userId: user._id,
      });

      if (updatedUserAddresses.addresses.length > 0) {
        const newDefaultAddress = updatedUserAddresses.addresses[0];
        updatedUserAddresses.defaultAddress = newDefaultAddress;

        // Save the updated document
        await updatedUserAddresses.save();
      } else {
        // If no addresses are left, set defaultAddress to null
        updatedUserAddresses.defaultAddress = null;
        await updatedUserAddresses.save();
      }
    }
