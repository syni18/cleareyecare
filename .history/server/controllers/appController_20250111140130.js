...
export async function addItemsToCart(req, res) {
  try {
    ...
    let cart = await ShoppingBagModel.findOne({ userId: user._id });

    if (!cart) {
      // If no cart exists, create a new one
      cart = new ShoppingBagModel({
        userId: user._id,
        items: [],
        totalPrice: 0,
      });
    ...
    res
      .status(200)
      .json({ status: true, cart: isCart msg: "Product added to the cart successfully." });
  } catch (error) {
