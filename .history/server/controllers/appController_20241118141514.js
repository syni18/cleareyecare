...
export async function getCallbackGoogle(req, res, next) {
  try{
      ...
      (req, res, next) => {
        // access user
        const { user, accessToken, refreshToken, accessTokenExpiry, refreshTokenExpiry } = req.user;
        setTokenCookies(res, accessToken, refreshToken, accessTokenExpiry, refreshToken);
...
export async function register(req, res) {
    try {
        ...
        isUserExist.then((user) => {
          ...
          .catch((error) => {
            res.status(500).json({ msg: "Internal Server error", error: "Database error", details: error });
          });
    } catch (error) {
            ...
            .then((user) => {
                ...
                bcrypt.compare(password, user.password)
                    .then((passwordMatch) => {
                        ...
                        // Create JWT token
                        const { accessToken, refreshToken, accessTokenExpiry, refreshTokenExpiry } = aw generateJWT(user);

                        console.log("token", accessToken, refreshToken, accessTokenExpiry);
