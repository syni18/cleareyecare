...
export async function manageAddress(req, res) {
  ...
  try {
    ...
    if (mode === "create") {
      ...
      const newAddress = {
        id: uuidv4(), // Unique ID for the new address
        ...values,
      };
      ...
      if (userAddressData) {
        ...
        if (!userAddressData.defaultAddress) {
          userAddressData.defaultAddress = newAddress;
        }
...
export async function getUserAddresses(req, res) {
  try {
    ...
    const isAddressExist = await AddressModel.find({ userId: user._id });
    console.log("Addresses found:", isAddressExist.addresses);
