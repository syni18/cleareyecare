...
export async function getNewAccessToken(req, res) {
  try {
    const {
      newAccessToken, newRefreshToken, newAccessTokenExpiry, newRefreshTokenExpiry} = await refreshAccessToken(req, res);
    // set cookies new
...
export async function register(req, res) {
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ status: true, msg: "Email address already exists." });
    }
...
export async function recovery(req, res, next) {
  ...
  try {
    ...
    return  res.status(201).send({userId: isUserExist._id, msg: "OTP sent to your email." });
...
export async function resetPassword(req, res) {
  try {
    ...
    if (password !== confirmPassword) {
      return res.status(400).send({ msg: "New passwords do not match" });
    }

    const user = await UserModel.findById(id);

    if (!user) {
      return res.status(404).send({ msg: "Unauthorized access attempt" });
    }

    const hashedPassword = await bcrypt.hash(password, 10);

    // Update the user's password
    user.password = hashedPassword; // Directly update the user's password
