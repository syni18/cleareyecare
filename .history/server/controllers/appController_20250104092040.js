...
import "../encryption/AESencryption.js"
...
export async function pancardvalidation(req, res) {
  try {
    ...
    // Perform a single query to check for existing PAN card or PAN number
    const existingPanRecord = await PancardModel.findOne({
      $or: [{ userId: user.id }, { panNumber: values.panNumber }],
    });

    if (existingPanRecord) {
      // Check if the PAN card is already associated with the user
      ...
      if (existingPanRecord.panNumber === values.panNumber) {
        return res
          .status(400)
    ...
    const newPancard = new PancardModel({
      userId: user.id, // User ID from authenticated request
      fullname: values.fullname,
      panNumber: values.panNumber,
      panImage: values.panImage, // Base64 encoded image
