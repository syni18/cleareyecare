...
export async function removeItemWishlists(req, res) {
  try {
    ...
    if (isWishlistExists) {
      ...
      const productIndex = isWishlistExists.items.findIndex(
        (item) => item.productId.toString() === productId
      );

      if (productIndex !== -1) {
        // Remove the product from the items array
        isWishlistExists.items.splice(productIndex, 1);
        await isWishlistExists.save(); // Save the updated wishlist

        return res.status(200).json({
          status: true,
          msg: "Product removed from wishlist successfully",
          updatedWishlist: isWishlistExists, // Return the updated wishlist
        });
      } else {
...
export async function getWishlists(req, res) {
  try {
    ...
    const item = isWishlistExist.items.map((item) => items.productId)
    console.log("isWishlistExist", isWishlistExist);
    
    return res.status(200).json({ wishlist: isWishlistExist });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
