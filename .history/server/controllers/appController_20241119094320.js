...
export async function recovery(req, res, next) {
  ...
  try {
    ...
    await new RecoveryOTPModel({userId: isUserExist._id, otp: otp}).save();

    // send email notification
    const emailBody = otpMessage();
    const subject = `Reset Password`
    await sendEmai(email, subject, emailBody);
    return  res.status(201).send({ msg: "OTP sent to your email." });
