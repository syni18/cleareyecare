...
export async function pancardvalidation(req, res) {
  try {
    ...
    const encryptedPanNumber = encryptionText(values.panNumber.number);
    const encryptedPanImage = encryptionText(values.panImage.image);

    // Check for existing PAN card or PAN number
    const existingPanRecord = await PancardModel.findOne({
      $or: [{ userId: user.id }, { panNumber: encryptedPanNumber.encrypt }],
    });
    ...
    if (existingPanRecord) {
      if (existingPanRecord.userId.toString() === user.id) {
        return res
          .status(400)
          .json({ error: "User has already saved a PAN card." });
      }

      if (existingPanRecord.panNumber.number === encryptedPanNumber) {
        return res
          .status(400)
          .json({ error: "This PAN number is already registered." });
      }
    }
    ...
    const newPancard = new PancardModel({
      ...
      panNumber.secret.iv: encryptedPanNumber.iv,
      panNumber.secret.key: encryptedPanNumber.key,
      panImage: encryptedPanImage,
      panImage.secret.iv: encryptedPanNumber.iv,
      panImage.secret.key: encryptedPanNumber.key,
    ...
    console.log("PAN card information saved successfully.");
    return res.status(201).json({
      message: "PAN card information saved successfully.",
      pancard: newPancard,
    });
  } catch (error) {
...
export async function getPancardDetails(req, res) {
  try {
    ...
    const data = {
      ...
      panImage: decryptionText(isPanExist.panImage),
      panNumber: decryptionText(isPanExist.panNumber),
