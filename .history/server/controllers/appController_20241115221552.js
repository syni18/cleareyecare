...
//
export async function getAuthGoogle(req, res) {
  try {
    const result = passport.authenticate("google", { scope: ["profile", "email"] });
    console.log("res", result.authenticate);
    return result.json;
  } catch (error) {
...
// 
export async function getCallbackGoogle(req, res) {
  try{
            ...
            if (existingUser) {
                ...
                bcrypt.hash(password, 10).then((hashedPassword) => {
                    ...
                    user.save().then((result) => {
                        res.status(201).json({userId:result._id, message: "User registered successfully" });
                    }).catch((error) => {
