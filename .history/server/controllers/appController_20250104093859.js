...
import { encryptionText } from "../encryption/AESencryption.js";
...
export async function pancardvalidation(req, res) {
  try {
    ...
    const existingPanRecord = await PancardModel.findOne({
      $or: [{ userId: user.id }, { panNumber: encryptedPanNumber }],
    });
...
export async function showPancardDetails(req, res) {
  try {
    const user = req.user;
    const isPanExist = await PancardModel.findOne({userId: user.id});
    if(isPanExist) {
      console.log("show data", isPanExist);
      
      // const data = 
    }
    return {data: isPanExist};
  } catch (e) {
    console.error("Error fetching PAN card details:", e);
    return res.status(500).json({ error: "Internal Server Error" });
  }
}
