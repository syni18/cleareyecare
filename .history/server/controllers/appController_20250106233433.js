...
export async function addItemWishlists(req, res) {
  try {
    const user = req.user;
    const { productId, notes } = req.body; // Extract productId and optional notes from the request body
    ...
    if (isWishlistExists) {
      ...
      if (isProductExists) {
        ...
        isWishlistExists.items.push({
          productId,
          notes: notes || "", // Add notes if provided, otherwise default to an empty string
        });
      ...
      return res
        .status(201)
        .json({ message: "Wishlist created and product added successfully" });
    }
  } catch (error) {
    console.error(error);
