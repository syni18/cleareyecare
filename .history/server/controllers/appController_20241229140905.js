...
export async function deleteAddressById(req, res) {
  try {
    const user = req.user;
    const { id } = req.params; // Assuming userId and addressId are provided in the request body
    // console.log(req.body);
    if (!user) {
      return res.status(404).json({ error: "Unauthorized Access" });
    }
    // Use $pull to remove the specific address from the array
    const result = await AddressModel.updateOne(
      { userId: user._id }, // Find the document by userId
      { $pull: { addresses: { id: addressId } } } // Remove the address matching the addressId
    );
