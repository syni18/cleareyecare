  ...
  try {
    ...
    if (!product || !product._id) {
      return res
        .status(400)
        .json({
          error: "Product object or ID is missing in the request body.",
        });
    }

    // Check if the user's cart exists
    const cart = await ShoppingBagModel.findOne({ userId: user._id });
    ...
    res
      .status(200)
      .json({ message: "Product removed from the cart successfully.", cart });
  } catch (error) {
